...
CURRENT_SESSION_GND_DIR = None # This will be set dynamically
    ...
    if len(faces) > 0:
        ...
        for (fx, fy, fw, fh) in faces:
            ...
            if len(current_face_eyes) >= 2:
                current_face_eyes.sort(key=lambda coord: coord[0]) 
...
def detect_eyes_single_image(image_filepath, face_cascade, eye_cascade, scaleFactor, minNeighbors):
    """
    Detects faces and then eyes in a single image with given parameters.
    Returns detected coordinates (left_eye, right_eye) or None.
    (No file I/O for speed during grid search.)
    """
    img_filename = os.path.basename(image_filepath)
    img = cv2.imread(image_filepath)
    if img is None:
...
def tune_haar_parameters(
    ...
    """
    Tunes Haar cascade parameters (scaleFactor, minNeighbors) to find the best performance.
    Visualizes performance in a 3D graph.
    (No file I/O during grid search for speed.)
    """
    best_f1 = -1
    best_params = {}
    all_results = []
    ...
    for i, sf in enumerate(scale_factor_range):
        for j, mn in enumerate(min_neighbors_range):
            ...
            print(f"\n--- Testing Parameters: scaleFactor={sf_val:.2f}, minNeighbors={mn_val} ---")
            
            detected_coords_for_params = {}
            for filename in image_filenames:
                image_filepath = os.path.join(IMG_DIR, filename)
                
                detected_result = detect_eyes_single_image(
                    image_filepath, face_cascade, eye_cascade, sf_val, mn_val
                )
                if detected_result:
            ...
            print(f"\nMetrics for sf={sf_val:.2f}, mn={mn_val}:")
    ...
    print("\n--- All Tuning Results ---")
    # Sort results by F1-Score for better readability
    ...
    if best_params:
        print(f"Best F1-Score: {best_params['F1-Score']:.4f}")
        ...
        print("No successful detections found for any parameter set.")

    # --- 3D Visualization ---
    fig = plt.figure(figsize=(12, 10), dpi=300) # High definition
    ...
    if best_params:
        ax.scatter(best_params['scaleFactor'], best_params['minNeighbors'], best_params['F1-Score'], 
                   color='red', marker='o', s=100, label=f"Best F1: {best_params['F1-Score']:.2f}", depthshade=True)
    ...
    # Identify peaks (higher F1) and valleys (lower F1)
    # The viridis colormap goes from purple (low) to yellow (high).
    # So, yellow areas are "peaks" (better performance) and purple areas are "valleys" (worse performance).

    plt.legend()
    plt.tight_layout()

    # Save the plot
    graph_filename = f"3D_F1_Performance_Tuning_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
...
if __name__ == "__main__":
    ...
    # --- Prepare Ground Truth Data (simulated as in Q3) ---
    ...
    if face_cascade.empty():
        print(f"Error: Face cascade XML not loaded from {os.path.join(HAARCASCADE_DIR, 'haarcascade_frontalface_default.xml')}. Check path and file integrity.")
        exit()
    if eye_cascade.empty():
        print(f"Error: Eye cascade XML not loaded from {os.path.join(HAARCASCADE_DIR, 'haarcascade_eye.xml')}. Check path and file integrity.")
        exit()

    print("\n--- Starting Parameter Tuning ---")
    print("Testing different scaleFactor and minNeighbors combinations.")

    # Define the expanded ranges for tuning
    ...
    tune_haar_parameters(
        ...
        matching_threshold=20 # Pixels, defined in Q3
    )
